...
CrtExp06Check::CrtExp06Check(StringRef Name, ClangTidyContext *Context)
    : ClangTidyCheck(Name, Context),
      IgnoreMacros(Options.getLocalOrGlobal("IgnoreMacros", true)) {}

void CrtExp06Check::storeOptions(ClangTidyOptions::OptionMap &Opts) {
  Options.store(Opts, "IgnoreMacros", IgnoreMacros);
}

void CrtExp06Check::registerMatchers(MatchFinder *Finder) {
...
void CrtExp06Check::check(const MatchFinder::MatchResult &Result) {
  ...
  if (MatchedDecl->getLocation().isInvalid()) return;

  SourceLocation StartLoc = MatchedDecl->getBeginLoc();

  if (StartLoc.isMacroID() && IgnoreMacros) return;
