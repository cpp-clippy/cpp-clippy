...
void CrtExp09Check::registerMatchers(MatchFinder *Finder) {
...
void CrtExp09Check::check(const MatchFinder::MatchResult &Result) {
...
void CrtExp09Check::addParm(const ParmVarDecl *Parm) {
...
void CrtExp09Check::setReferenced(const DeclRefExpr *Ref) {
  auto It = Parameters.find(dyn_cast<ParmVarDecl>(Ref->getDecl()));
  if (It != Parameters.end())
    It->second.IsReferenced = true;
}

void CrtExp09Check::onEndOfTranslationUnit() {
  diagnoseNonConstParameters();
}

void CrtExp09Check::diagnoseNonConstParameters() {
...
void CrtExp09Check::markCanNotBeConst(const Expr *E,
                                               bool CanNotBeConst) {
  if (!E)
    return;
