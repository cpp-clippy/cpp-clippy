...
CrtExp05Check::CrtExp05Check(StringRef Name, ClangTidyContext *Context)
  ...
  if (AnalyzeValues == false && AnalyzeReferences == false)
    this->configurationDiag(
        "The check 'critical-crt-exp-05' will not "
        "perform any analysis because both 'AnalyzeValues' and "
        "'AnalyzeReferences' are false.");
}

void CrtExp05Check::storeOptions(ClangTidyOptions::OptionMap &Opts) {
...
void CrtExp05Check::registerMatchers(MatchFinder *Finder) {
...
void CrtExp05Check::check(const MatchFinder::MatchResult &Result) {
  ...
  if (Variable->getType()->isReferenceType()) VC = VariableCategory::Reference;
  if (Variable->getType()->isPointerType()) VC = VariableCategory::Pointer;
  ...
  if (VC == VariableCategory::Reference && !AnalyzeReferences) return;
  ...
  if (VC == VariableCategory::Pointer && !WarnPointersAsValues) return;

  if (VC == VariableCategory::Value && !AnalyzeValues) return;

  // The scope is only registered if the analysis shall be run.
  registerScope(LocalScope, Result.Context);

  // Offload const-analysis to utility function.
  if (ScopesCache[LocalScope]->isMutated(Variable)) return;
  ...
  if (VarDeclStmt == nullptr || !VarDeclStmt->isSingleDecl()) return;
...
void CrtExp05Check::registerScope(const CompoundStmt *LocalScope,
                                  ASTContext *Context) {
