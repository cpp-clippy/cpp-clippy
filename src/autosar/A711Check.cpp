...
A711Check::A711Check(StringRef Name, ClangTidyContext *Context)
  ...
  if (AnalyzeValues == false && AnalyzeReferences == false)
    this->configurationDiag(
        "The check 'a7-1-1' will not "
        "perform any analysis because both 'AnalyzeValues' and "
        "'AnalyzeReferences' are false.");
}

void A711Check::storeOptions(ClangTidyOptions::OptionMap &Opts) {
...
void A711Check::check(const MatchFinder::MatchResult &Result) {
  ...
  auto Diag = diag(Variable->getBeginLoc(),
                   "variable %0 of type %1 can be declared 'constexpr' or 'const'")
...
void A711Check::registerScope(const CompoundStmt *LocalScope,
                                          ASTContext *Context) {
