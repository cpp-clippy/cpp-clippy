...
void M712Check::registerMatchers(MatchFinder *Finder) {
...
void M712Check::check(const MatchFinder::MatchResult &Result) {
...
void M712Check::addParm(const ParmVarDecl *Parm) {
...
void M712Check::setReferenced(const DeclRefExpr *Ref) {
  auto It = Parameters.find(dyn_cast<ParmVarDecl>(Ref->getDecl()));
  if (It != Parameters.end())
    It->second.IsReferenced = true;
}

void M712Check::onEndOfTranslationUnit() {
  diagnoseNonConstParameters();
}

void M712Check::diagnoseNonConstParameters() {
...
void M712Check::markCanNotBeConst(const Expr *E,
                                               bool CanNotBeConst) {
  if (!E)
    return;
